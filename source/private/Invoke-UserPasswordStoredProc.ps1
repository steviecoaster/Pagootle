function Invoke-UserPasswordStoredProc {
    ...
    Param(
        ...
        $ScriptBlock = {
            param ($ConnectionString, $Params, $GetDatabase)

            $ConnectionType = & $GetDatabase | % Type
        ...
        if ($UseRemoting) {
            ...
            $RemoteArgs = @{
                ...
                ArgumentList = $Connection.ConnectionString, $Params, (Get-Command Get-ProGetDatabase)
            }
            if ($Credential) {
            ...
            # Execute locally
            & $ScriptBlock $Connection.ConnectionString $Params (Get-Command Get-ProGetDatabase)
        }
    }
}